...
atlaschat.font 					= atlaschat.config.New("Chatbox font", 						"font", 			"atlaschat.verdana.se", true)
...
atlaschat.timestamp 			= atlaschat.config.New("Enable timestamp", 					"timestamp", 		false, 					true)
...
atlaschat.smallAvatar 			= atlaschat.config.New("Use small avatars", 				"small_avatar", 	true, 					true)
atlaschat.maxHistory 			= atlaschat.config.New("Max chat history", 					"max_history", 		25, 					true)
atlaschat.extraShadow 			= atlaschat.config.New("Extra font shadow", 				"extra_shadow", 	true, 					true)
...
----------------------------------------------------------------------
-- Function:
--		theme:ParseExpressions(data, panel, player, title)
-- Purpose:
...
function theme:ParseData(data, list, isTitle)
	...
	for i = 1, #data do
		...
		if (type == "Player") then
			...
			local color, text = data[i -1] and IsColor(data[i -1]) and realColor or team.GetColor(value:Team()), GAMEMODE.GetAmMurderer and value:Team() == 2 and value:GetBystanderName() or value:Nick()
	...
	elseif (GAMEMODE.round_state) then
		...
		if (GetType(data[4]) == "string" and GetType(data[2]) == "string") then
			if (check) then
				...
				if (#data == 4) then -- hopefully this won't need any further fix lol
					local player = util.FindPlayerAtlaschat(data[2])
