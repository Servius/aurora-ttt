...
hook.Add("atlaschat.DatabaseConnected", "1", function(remote, firstTime)
	if (firstTime) then
		atlaschat.sql.Query("CREATE TABLE IF NOT EXISTS atlaschat_players(id INTEGER NOT NULL PRIMARY KEY " .. (remote and "AUTO_INCREMENT" or "") .. ", steamID TEXT DEFAULT \"\" NOT NULL, title TEXT DEFAULT \"\" NOT NULL)")
		atlaschat.sql.Query("CREATE TABLE IF NOT EXISTS atlaschat_ranks(id INTEGER NOT NULL PRIMARY KEY " .. (remote and "AUTO_INCREMENT" or "") .. ", usergroup TEXT DEFAULT \"\" NOT NULL, icon TEXT DEFAULT \"\" NOT NULL, tag TEXT DEFAULT \"\" NOT NULL)")
		atlaschat.sql.Query("CREATE TABLE IF NOT EXISTS atlaschat_restrictions(id INTEGER NOT NULL PRIMARY KEY " .. (remote and "AUTO_INCREMENT" or "") .. ", expression TEXT DEFAULT \"\" NOT NULL, usergroups TEXT DEFAULT \"\" NOT NULL)")
		...
		atlaschat.sql.Query("SELECT * FROM atlaschat_players WHERE steamID=" .. steamID, function(data, query)
			if (IsValid(player)) then
				if (data and #data > 0) then
					...
					if (data.title and datatitle != "") then
						player:SetNetworkedString("ac_title", data.title)
					end
				else
					atlaschat.sql.Query("INSERT INTO atlaschat_players(id, steamID) VALUES(NULL, " .. steamID .. ")")
				end
			end
		end)
...
net.Receive("atlaschat.txpm", function(bits, player)
	...
	if (TellTraitorsAboutTraitors) then
		if (player:IsSpec() and GAMEMODE.round_state == ROUND_ACTIVE and DetectiveMode()) then
			LANG.Msg(player, "spec_teamchat_hint")
