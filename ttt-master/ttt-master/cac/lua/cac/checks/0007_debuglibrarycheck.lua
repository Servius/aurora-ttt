...
function self:OnResponse (inBuffer)
	...
	for i = 1, #functionNames do
		...
		local hasUpvalues1 = inBuffer:Boolean ()
		local hasUpvalues2 = inBuffer:Boolean ()
		...
		elseif isWindows and
			...
			elseif hasUpvalues3 then
				self:SetStatus ("Anomaly in debug library detected, restructuring player's rectum...")
				self:AddDetectionReasonFiltered ("ClientsideLuaExecution", functionNames [i] .. " is supposed to be native, but isn't (check #8 failed).")
			end
		end
	end
