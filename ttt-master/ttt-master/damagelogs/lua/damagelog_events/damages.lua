...
function event:EntityTakeDamage(ent, dmginfo)
	...
	if ent:IsPlayer() and (IsValid(att) and att:IsPlayer()) and ent != att then
		...
		if math.floor(damages) > 0 then
			...
			if (not string.find(tbl[6], "weapon_ghost")) then
				self.CallEvent(tbl)
			end
		end
	end
...
function event:ToString(tbl)
	...
	local str
	if weapon then
		str = string.format("%s [%s] damaged %s [%s] for %s HP with %s.", tbl[3], Damagelog:StrRole(tbl[4]), tbl[1], Damagelog:StrRole(tbl[2]), tbl[5], weapon) 
	else
		str = string.format("%s [%s] damaged %s [%s] for %s HP with an unknown weapon.", tbl[3], Damagelog:StrRole(tbl[4]), tbl[1], Damagelog:StrRole(tbl[2]), tbl[5]) 
	end
	return str
