...
function Damagelog:OpenMenu()
	...
	self.Menu:SetTitle("Damagelog Menu")
...
function Damagelog:CheckPrivileges()
	if not LocalPlayer():CanUseDamagelog() then
		chat.AddText(Color(204,0, 0, 255), "You cannot access the damagelog at the moment.")
		return false
	end
	return true
end
...
function Damagelog:Think()
	if input.IsKeyDown(KEY_F9) and not self.pressed_key then
	...
	elseif self.pressed_key and not input.IsKeyDown(KEY_F9) then
...
net.Receive("DL_Ded", function()
	...
	chat.AddText(Color(255,255, 255), "You've died! You can send a report by entering ", Color(204, 0, 0), Damagelog.RDM_Manager_Command)
