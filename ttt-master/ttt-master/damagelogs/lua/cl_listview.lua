...
function Damagelog:AddRoleLine(listview, nick, role)
	...
	item.Think = function(panel)
		...
		if GetRoundState() == ROUND_ACTIVE and sync_ent:GetPlayedRounds() == panel.Round then
			...
			panel:SetColumnText(3, "Round end")
		end
	end
end
		...
		for k,v in ipairs(nums) do
			...
			for s,i in pairs(info) do
				...
				if i[2] == "crowbartir" then
					item = listview:AddLine(string.format("%s - %s swung their crowbar", string.FormattedTime(v, "%02i:%02i"), i[1]))
				elseif i[2] == "crowbarpouss" then
					item = listview:AddLine(string.format("%s - %s pushed %s with a crowbar", string.FormattedTime(v, "%02i:%02i"), i[1], i[3]))
				else
					item = listview:AddLine(string.format("%s - %s shot an %s", string.FormattedTime(v, "%02i:%02i"), i[1], self.weapon_table[i[2]] or i[2] or ""))
				end
